...
def create_blank_frame(width, height, padding, color=(0, 0, 0)):
...
def add_caption_to_frame(_frame,_resolution,_fps,_project_name,_text_ts_info,_total_frame_number,_text_cut_num,_text_cut_take,_cut_status,_cut_staff,_cut_filedate,_local_frame_number,_video_index):
            ...
            if _cut_filedate[_video_index] != 'NoFile':
                drawText('left','center',_frame,_cut_filedate[_video_index],(1600,100),0.5)
            else:
    ...
    for dir in dirs:
        ...
        if len(os.listdir(dirPath)) != 0:
            for file in files:
                ...
                text_cut_filedate.append(updated_dt.strftime('%Y%m%d') if os.listdir(dirPath) else 'NoFile')
            ...
            #なにもなかった場合
            text_cut_filedate.append(updated_dt.strftime('NoFile'))
            duration =  (int(text_cut_length_second[video_index]) * fps) + int(text_cut_length_frame[video_index])
            local_frame_number = 0
            for _ in range(duration):
